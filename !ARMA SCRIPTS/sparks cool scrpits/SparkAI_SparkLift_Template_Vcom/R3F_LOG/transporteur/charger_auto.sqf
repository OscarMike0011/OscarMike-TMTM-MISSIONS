...
// Pour chaque élément de la liste à charger
{
	...
	if (typeName _objet_ou_classe == "STRING") then
	{
		...
		// Search for a clear position. Vehicles must be created at ground level otherwise they cannot be used.
	...
	// VUK: Make sure that the type of object to be loaded is transportable.
	...
	else
	{
		...
		{
			// VUK: If the object to load is given as a class name, create.
			// Si l'objet à charger est donné en tant que nom de classe, on le crée
			if (typeName _objet_ou_classe == "STRING") then
			{
				// VUK: Search for a clear position. Vehicles must be created at ground level otherwise they cannot be used.
				// Recherche d'une position dégagée. Les véhicules doivent être créé au niveau du sol sinon ils ne peuvent être utilisés.
				if (_classe isKindOf "AllVehicles") then
				{
			...
			{
				// VUK: Check that it is not already transported.
				...
				) then
				{
					...
					{
						// VUK: Check if the object is in the vehicle.
...
// VUK: Content is stored inside the vehicle and then broadcasted on the network.
