...
while {true} do
{
				...
				{
					{
						...
						!([_x, _joueur] call R3F_LOG_FNCT_objet_est_verrouille) && !(_x getVariable "R3F_LOG_disabled") &&
						{
							...
							abs (_delta_pos select 0) < 50 && abs (_delta_pos select 1) < 50
						}
	...
	) then
	{
			...
			{
				{
					...
					((getPosASL _vehicule_joueur select 2) - (getPosASL _x select 2) > 5 && (getPosASL _vehicule_joueur select 2) - (getPosASL _x select 2) < 50)
				} count (nearestObjects [_vehicule_joueur, ["All"], 50]) != 0
			};
		...
		R3F_LOG_action_heliport_larguer_valide = !isNull (_vehicule_joueur getVariable "R3F_LOG_heliporte") && !(_vehicule_joueur getVariable "R3F_LOG_disabled") &&
			(vectorMagnitude velocity _vehicule_joueur < 40) && ((getPosASL _vehicule_joueur select 2) - (0 max getTerrainHeightASL getPos _vehicule_joueur) < 60);
	}
	else
	{
