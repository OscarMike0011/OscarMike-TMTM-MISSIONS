...
else
{
	...
	_objet = objNull;
	{
		if (
			...
			((getPosASL _heliporteur select 2) - (getPosASL _x select 2) > 5 && (getPosASL _heliporteur select 2) - (getPosASL _x select 2) < 50)
		) exitWith {_objet = _x;};
	} forEach (nearestObjects [_heliporteur, ["All"], 75]);
					...
					if (isNull (_objet getVariable ["R3F_LOG_remorque", objNull])) then
					{
						...
						while {_attente_valide && time - _time_debut < _duree} do
						{
							...
							if !(
								...
								isNull (_heliporteur getVariable "R3F_LOG_heliporte") && (vectorMagnitude velocity _heliporteur < 50) && (_heliporteur distance _objet < 50) &&
								...
								((getPosASL _heliporteur select 2) - (getPosASL _objet select 2) > 5 && (getPosASL _heliporteur select 2) - (getPosASL _objet select 2) < 50)
							) */
							if !(
								alive player && vehicle player == _heliporteur && !(_heliporteur getVariable "R3F_LOG_disabled") && _pas_de_hook &&
								isNull (_heliporteur getVariable "R3F_LOG_heliporte") && (vectorMagnitude velocity _heliporteur < 50) && (_heliporteur distance _objet < 50) &&
						...
						if (_attente_valide) then
						{
							...
							[_heliporteur, ["<t color='#ffdd02'>Paradrop Pod</t>", "R3F_LOG\toutenbas.sqf",[_objet],2,false,true,"", "driver _target == _this"]] remoteExec ["addAction", 0, false];
